...
$newНазвание = $_POST['название'];
  ...
  $updateSql = "UPDATE `арендатор юрлицо` SET `Название юр лица` = '$newНазвание', `ИНН/КПП` = '$newИННКПП', `Банковские реквизиты` = '$newРеквизиты', `Адрес` = '$newАдрес', `email` = '$newEmail', `Контактное лицо` = '$newКонтактное_лицо' WHERE `email` = '$id'";
  $db->Execute($updateSql);
  echo '<script>window.location.href = "./arendur.php";</script>';
}
if ($info == null){
  $sql3 = "SELECT * FROM `arendfiz` WHERE `email` = '$id'";
  ...
  // Выполняем обновление записи в базе данных
  $updateSql = "UPDATE `arendfiz` SET `fam` = '$newFam', `name` = '$newName', `otch` = '$newOtch', `tel` = '$newTel', `email` = '$newEmail' WHERE `email` = '$id'";
  $db->Execute($updateSql);
  echo '<script>window.location.href = "./arendfiz.php";</script>';
}
